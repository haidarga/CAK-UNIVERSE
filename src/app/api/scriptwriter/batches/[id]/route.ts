...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('batches').select('*').eq('id', id).eq('created_by', user.id).maybeSingle()
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('batches').update(patch).eq('id', id).eq('created_by', user.id).select('*').maybeSingle()
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 })
  if (!data) return NextResponse.json({ ok: false, error: 'not found' }, { status: 404 })
  return NextResponse.json({ ok: true, batch: data })
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: batch } = await authClient.from('batches').select('id').eq('id', id).eq('created_by', user.id).maybeSingle()
  ...
  const { error: nErr } = await service.from('naskah').delete().eq('batch_id', id).eq('created_by', user.id)
  if (nErr) return NextResponse.json({ ok: false, error: nErr.message }, { status: 500 })
  const { error: bErr } = await service.from('batches').delete().eq('id', id).eq('created_by', user.id)
