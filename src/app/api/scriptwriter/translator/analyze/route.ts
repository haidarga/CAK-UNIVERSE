...
import { translateImageToDirection, isSupportedImageMime } from '@/lib/cakgpt/vision-translate'
...
const ANALYZE_DEADLINE_MS = 60_000
const MAX_NOTE_LEN = 1000

// POST /api/scriptwriter/translator/analyze — Content Translator. The image
// was already uploaded straight to Supabase Storage via a signed URL (same
// flow as briefs/naskah import — see /imports/upload-url); this route just
// receives the storage path, downloads + analyzes it, then deletes the file
// (transient, one-shot — never persisted).
//
...
async function handleAnalyze(req: Request) {
  ...
  if (!isSupportedImageMime(mimeType)) {
    return NextResponse.json({ ok: false, error: `unsupported image type: ${mimeType || '(none)'}` }, { status: 415 })
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: e instanceof Error ? e.message : 'failed to read the uploaded image' }, { status: 400 })
  } finally {
  ...
  try {
    const result = await withDeadline(
      translateImageToDirection({ apiKey, imageBase64, mimeType, note }),
      ANALYZE_DEADLINE_MS,
      'image analysis',
    )
    ...
    const msg = e instanceof DeadlineExceededError
      ? 'this analysis is taking too long — try again.'
